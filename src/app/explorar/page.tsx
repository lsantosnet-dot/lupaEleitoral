...
function ExplorarContent() {
  ...
  const getFilteredPoliticios = (cargoFilter: string) => {
    return mockData.politicos.filter(
      (p) => p.cargo.toLowerCase() === cargoFilter && 
  ...
  const renderList = (list: typeof mockData.politicos) => {
    if (list.length === 0) {
            ...
            <Card key={p.id} className="border-slate-200 shadow-sm overflow-hidden flex flex-col pt-0">
              <CardContent className="p-4 flex gap-4 relative">
                <button className="absolute top-4 right-4 text-slate-400 hover:text-rose-500 transition-colors">
                  <Heart className={`h-5 w-5 ${p.isFavorito ? 'fill-rose-500 text-rose-500' : ''}`} />
                </button>
                ...
                <div className="flex-1">
                  <Link href={`/perfil/${p.id}`} className="block group">
                      ...
                      <p className="text-xs text-slate-500 mb-2">{p.partido} - {p.uf}</p>
                  </Link>

                  <div className="flex gap-4 mb-3">
                      <div>
                        <span className="text-[10px] text-slate-500 uppercase tracking-wider block mb-0.5">Assiduidade</span>
                        <span className={`text-sm font-medium ${p.presenca_pct >= 90 ? 'text-emerald-600' : p.presenca_pct >= 80 ? 'text-amber-600' : 'text-rose-600'}`}>
                          {p.presenca_pct}%
                        </span>
                      </div>
                      <div>
                        ...
                        <span className="text-sm font-medium text-slate-900">
                          R$ {(p.despesas_mes / 1000).toFixed(1)}k
                        </span>
                      </div>
                  </div>
...
export default function Explorar() {
  return (
    <div className="flex flex-col gap-4 p-4 pb-24">
      ...
      <Suspense fallback={<div className="text-center py-10 opacity-50">Carregando...</div>}>
         <ExplorarContent />
      </Suspense>
    </div>
  )
